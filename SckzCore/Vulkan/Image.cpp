...
namespace sckz {
    ...
    void Image::CreateTextureImage(const char * fileName, VkDevice & device, VkPhysicalDevice & physicalDevice, VkCommandPool & pool, VkQueue & queue) {
        this->device = & device;
        ...
        stagingBuffer.CreateBuffer(*this->physicalDevice, *this->device, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, queue); // queue is the PRIVATE queue

        stagingBuffer.CopyDataToBuffer(pixels, static_cast<size_t>(imageSize));
        stbi_image_free(pixels);

        CreateImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *this->device, *this->physicalDevice, queue); // queue is the PRIVATE queue
